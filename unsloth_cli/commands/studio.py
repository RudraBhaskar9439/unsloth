...
# Resolve install root: UNSLOTH_STUDIO_HOME, then the STUDIO_HOME alias, then sys.prefix inference (so a direct call to <root>/bin/unsloth resolves after the installer's env var has expired), then legacy ~/.unsloth/studio. Both halves, and the 8 KB ceiling, are Test-UnslothCmdShimFile's in install.ps1 and _IsUnslothCmdShim's in scripts/uninstall.ps1. Bytes, not text: the shim is written without a BOM but an edited copy may carry one, and a decode error here would be indistinguishable from "not ours".
...
def _looks_like_installer_managed_studio_home(candidate: Path) -> bool:
    """Sentinel check (studio.conf or bin shim) so a dev venv named unsloth_studio is not misidentified as a custom Unsloth root.

    On Windows bin\\unsloth.cmd counts too: only install.sh writes share/studio.conf, so on a custom-root Windows install the generated unsloth.exe is the only sentinel there is, and antivirus quarantine deletes it, after which this returns False, the root falls back to ~/.unsloth/studio, and every `unsloth studio ...` reads and writes the wrong installation. It has to be OUR .cmd: this decides which tree the CLI manages, and the directory is on PATH, so any file of that name would otherwise point a custom root at itself. Same marker Test-UnslothCmdShimFile in install.ps1 and the uninstaller's recursive-delete guard require.
    """
...
def _is_managed_cmd_shim(path: Path) -> bool:
    """Whether *path* is the .cmd shim this installer generates. Read as bytes and matched on the marker line install.ps1 writes: a hand-rolled wrapper that happens to invoke the CLI is not this, and must not be taken as proof of an installer-managed root."""
    try:
...
def _ensure_studio_env_exported() -> None:
    """Re-export UNSLOTH_STUDIO_HOME / UNSLOTH_LLAMA_CPP_PATH only for real custom roots so subprocesses inherit the right install. Called from each studio subcommand entry rather than at import time, to avoid leaking env state into unrelated importers (tests, --help, CLI introspection)."""
...
# __file__ is unsloth_cli/commands/studio.py, so two parents up is the package root (site-packages, or the repo root for editable installs).
...
# Windows materialises the `unsloth` entry point as a generated, unsigned .exe that Application Control denies while the signed interpreter beside it still runs, so every managed invocation goes through the interpreter (#8490). Byte-identical to WINDOWS_CLI_ENTRYPOINT in studio/src-tauri/src/process.rs and to $script:UnslothCliTrampoline in install.ps1, which carries the full rationale for both halves and for the deliberate absence of -I.
...
# Asks the managed interpreter whether the trampoline's import would succeed, by performing that exact import. The sys.path[0] scrub is _WINDOWS_CLI_ENTRYPOINT's, kept a separate literal because a parity test literal_evals that constant; the two must agree or the probe answers for a different sys.path than the launch uses. `from unsloth_cli import app` rather than a find_spec, deliberately: find_spec locates without executing, which answers True for an empty unsloth_cli/ directory (a namespace package), for a package whose __init__ raises, and for one whose dependencies an interrupted install never fetched. Every one of those is a venv the trampoline cannot start, and this probe gates the headless-public strip of .bootstrap_password, so a false pass there is a public Unsloth with no login page and no plaintext recovery credential.
_MANAGED_CLI_IMPORT_PROBE = (
    "import sys, os; sys.path[:1] = [x for x in sys.path[:1] if getattr(sys.flags, 'safe_path', False) or x not in ('', os.getcwd())]; "
    "from unsloth_cli import app; sys.exit(0)"
)

# Seconds, and generous: this is a bare interpreter start plus the CLI package import, and it can run from cold on a machine whose antivirus is scanning the venv it just quarantined a file out of. The caller treats a timeout as "no verdict", so overrunning it is not fatal.
_MANAGED_CLI_IMPORT_PROBE_TIMEOUT = 60

# CreateProcess refuses a program blocked by an Application Control policy with ERROR_ACCESS_DISABLED_BY_POLICY, which Python surfaces as OSError.winerror.
...
def _managed_cli_argv(
    ...
    *args: str,
    ...
    -X utf8 rather than PYTHONUTF8 so the encoding holds even for a caller that scrubs the environment, and it precedes -I because -I implies -E, which would discard PYTHONUTF8 but cannot touch a command-line -X. *isolated* mirrors the Isolation enum in studio/src-tauri/src/process.rs and carries the same rule: the default is inherit, because -I implies -E and -s and so drops PYTHONPATH, PYTHONWARNINGS, PYTHONHASHSEED and user site-packages that the console script honoured, an observable difference on machines with no policy at all. Only a caller reproducing an already-isolated launch asks for it, today the desktop updater's health probe.
    """
...
def _is_application_control_block(error: OSError) -> bool:
    """True when Windows refused to start a program because a policy blocks it. Distinct from a missing or corrupt executable: nothing ran, so the failure says nothing about the program itself."""
...
def _stream_for_subprocess(stream):
    """Return *stream* if it has a real OS file descriptor, else None. subprocess.run on Windows refuses to inherit std handles unless they are passed explicitly (otherwise close_fds=True forces bInheritHandles=False and a CREATE_NO_WINDOW child ends up with no stdio at all), so hand a real fd-backed stream through; when it has been captured by a test harness, fall back to None and let subprocess use its default."""
    if stream is None:
        return None
    try:
        stream.fileno()
...
def _openable_host_for_bind(run_mod, host: str) -> str:
    """The host for a URL we tell the user to open: the LAN address when one resolves, else loopback. A wildcard bind with no LAN address (WSL NAT, loopback-only) must never be printed as-is, since no browser can open it."""
...
def _managed_cli_site_packages_layout(python: Path) -> bool:
    """On-disk hint that the venv holding *python* still carries the CLI. Weaker than the import probe below and only used when the probe could not be run at all: an empty ``unsloth_cli/`` or an orphaned dist-info left by an interrupted install answers yes here without being importable. The dist-info is accepted alongside the package directory because an editable install of the checkout leaves a .pth and no unsloth_cli/ here."""
...
def _managed_cli_package_present(python: Path) -> bool:
    ...
    Windows only, and only asked when the console script is gone: the generated unsloth.exe is what proves a CLI on POSIX, but on Windows nothing launches it any more (#8490) and antivirus quarantine deletes the unsigned stub while leaving the environment perfectly able to run.

    Asked of the interpreter rather than of site-packages, because the two are not the same claim. What Windows launches is the trampoline's ``from unsloth_cli import app``, so an orphaned ``unsloth-*.dist-info`` or an empty ``unsloth_cli/`` directory is metadata, not a runnable CLI. It matters here specifically: this gate stands in front of the headless-public strip of .bootstrap_password, so passing a venv that then fails to import is the one outcome the gate's placement exists to prevent. The probe runs the trampoline's own import rather than a cheaper spec lookup (see _MANAGED_CLI_IMPORT_PROBE), with the same sys.path[0] scrub so an ``unsloth_cli`` directory in the caller's cwd cannot answer for the venv.
    """
    if platform.system() != "Windows":
        return False
    try:
        probe = subprocess.run(
            ...
            # Same as every other managed-interpreter probe here: a non-interactive Windows launch must not flash a console window.
            **_windows_hidden_subprocess_kwargs(),
        )
    except subprocess.TimeoutExpired:
        # Slow is not broken. A cold venv under an antivirus scan can take longer than the probe waits, and the re-exec this gate stands in front of has no timeout at all, so fall back to the on-disk layout rather than aborting an install that works.
        return _managed_cli_site_packages_layout(python)
    except (OSError, subprocess.SubprocessError):
        # No verdict for a different reason: the interpreter would not start at all (missing, or denied by an Application Control policy). The re-exec runs THAT interpreter, so it is going to fail the same way, and the on-disk layout cannot say otherwise. Fail closed, because the caller strips .bootstrap_password on a headless public launch before it re-execs: passing here would leave a public Unsloth with no login page and no plaintext recovery credential.
        return False
    return probe.returncode == 0


def _hsa_override_gfx_arch(value: Optional[str]) -> Optional[str]:
    """gfx arch named by an HSA_OVERRIDE_GFX_VERSION value, or None if unreadable. libhsakmt (topology.c) reads it as a major.minor.stepping triple (``sscanf(envvar, "%u.%u.%u%c") != 3`` rejects anything else) and the target name concatenates the stepping in hex, which is why 9.0.10 is gfx90a: 11.0.0 gives gfx1100, 11.5.1 gives gfx1151. Kept in sync with _hsa_override_gfx_arch in studio/install_python_stack.py and install.sh."""
    if not value:
        return None
...
def _torch_requires_rocm_metapackage(venv_dir: Path) -> bool:
    """Whether the installed torch actually resolves through the ``rocm`` meta-package. AMD's per-gfx wheels depend on it; the generic pytorch.org ROCm wheels vendor their own runtime and depend on nothing, so after a switch between them the meta-package is left behind describing a family with no bearing on what torch loads. Unknown shapes answer False: refusing to arbitrate leaves the environment untouched."""
            ...
            for info in sp.glob("torch-*.dist-info"):
                ...
                for line in text.splitlines():
                    if not line.lower().startswith("requires-dist:"):
                        continue
                    # `Requires-Dist: rocm[libraries,devel]==7.13.0` and plain `rocm` both count; `rocm-sdk-core` does not, being a component rather than the arbiter.
...
def _installed_rocm_single_arch(venv_dir: Path) -> Optional[str]:
    ...
    AMD's per-gfx index ships one runtime distribution per architecture, ``rocm-sdk-libraries-<family>``, and the torch beside it holds code objects for that family alone. Which one is live has to come from the ``rocm`` meta-package, whose ``Requires-Dist`` names the family AMD's torch resolved (verified on repo.amd.com/rocm/whl/gfx1151). Globbing for a ``rocm_sdk_libraries_gfx*`` directory instead would read an ORPHAN: ``rocm`` upgrades in place across a family switch while the superseded runtime keeps its own distribution name and is never uninstalled. Same reasoning and hazard as _installed_rocm_wheel_family in studio/install_python_stack.py.

    None means "do not act": no ``rocm``, unreadable metadata, more than one family named, or a MULTI-arch family such as gfx120x-all, whose runtime carries kernels for several ISAs and so contradicts no override.
    """
    # The bare `rocm` metadata only describes a LIVE install while torch still resolves through it. Switching from AMD's per-gfx index to a generic pytorch.org one does not uninstall it: the generic wheels vendor their own ROCm libraries and depend on no meta-package, so `rocm` is orphaned and pip never removes it. Reading it then would name the OLD family and clear an override the generic wheels may be the only reason the GPU works at all.
    ...
    for sp_pattern in ("lib/python*/site-packages", "Lib/site-packages"):
        for sp in venv_dir.glob(sp_pattern):
            for info in sp.glob("rocm-*.dist-info"):
                # rocm-sdk-core and rocm-sdk-libraries-* also start "rocm-"; only the bare `rocm` meta-package arbitrates.
                if (
    ...
    # Single ISA only: gfx120x-all style families cover several architectures, so an override naming one of them is contradicted by nothing.
...
def _clear_hsa_override_contradicting_install(venv_dir: Path) -> Optional[str]:
    ...
    libhsakmt (topology.c) writes the variable's major.minor.stepping straight into the KFD node's EngineId and ROCr names the agent from that, so the override decides the ISA every later process sees. Against per-gfx wheels, which hold code objects for one architecture, an override naming a different one leaves the runtime asking for kernels the install does not contain and every launch fails on the first allocation.

    install.sh clears it for the one launch it performs itself, but that unset dies with the installer: `unsloth studio update` runs install_python_stack.py as a child and every later launch inherits the user's shell instead. This is the chokepoint the exec, the Windows Popen and the in-process paths all pass through. Keyed on the INSTALL, never on a hardware probe: on a generic multi-arch index there is nothing to contradict and the override is often the only thing making the GPU usable. Returns the installed arch when the variable was dropped.
    """
...
def _clear_hsa_override_before_launch(silent: bool = False) -> Optional[str]:
    """Run the #7331 spoof clear for whichever entry point is about to launch. Every launch needs it, not just plain ``unsloth studio``: the group callback returns early once a subcommand is named, and ``unsloth run`` is bound straight to ``studio_run``, so both would otherwise reach llama-server and the backend with the contradicting override still set. Idempotent."""
...
def _find_run_py() -> Optional[Path]:
    """Find studio/backend/run.py, with no CWD dependency. studio/ is a proper package, so it lives in site-packages after pip install, right next to unsloth_cli/."""
...
def _install_state(deep: bool = False) -> dict:
    """verify_install() result for this install root. STUDIO_HOME is an extra search root so a CLI installed outside the managed venv still inspects the venv the desktop app launches."""
...
def _load_run_module():
    """Import studio.backend.run without relying on package resolution: `studio update` can leave a partial ``site-packages/studio/backend/`` tree of plugin build artefacts, which wins over an editable install and breaks ``from studio.backend.run import ...``. Loading by file path sidesteps the conflict."""
    global _RUN_MODULE
...
def _find_setup_script(repo_root: Optional[Path] = None) -> Optional[Path]:
    """Find studio/setup.sh or studio/setup.ps1, with no CWD dependency. `repo_root` is the explicit --local checkout when there is one, and its setup script has to win: the scripts build the frontend under their own $SCRIPT_DIR, and the editable install of `repo_root` removes the installed tree that the installed copy's script would have built into. studio/frontend/dist is gitignored, so a fresh checkout would then have no frontend at all."""
    name = "setup.ps1" if platform.system() == "Windows" else "setup.sh"
    # The checkout the caller actually asked to install from. No fallback: dropping back to the installed copy's script is the exact behaviour this branch exists to stop, so a checkout without one is unusable rather than a reason to use somebody else's.
...
# Mirror in studio/backend/run.py argparse plus the backend denylist test; bumping the cap in one place only desyncs.
_PARALLEL_MIN = 1
_PARALLEL_MAX = 64
_PARALLEL_DEFAULT_RUN = 4  # pre-PR hardcoded for `unsloth studio run`
# New Chat leaves the previous conversation generating and the admission queue caps decodes at the slot count, so at 1 every extra chat queues. _slots_that_fit_on_gpu() may cut it back.
_PARALLEL_DEFAULT_PLAIN = 4


def _resolve_secure(secure: bool, not_secure: bool) -> bool:
    """Reconcile the deprecated --not-secure alias with --secure/--no-secure. Typer parses --secure and --not-secure as independent options, so the alias cannot lean on Click's last-wins ordering the way --secure/--no-secure do. Restore that ordering from argv: --not-secure only forces secure off when it is the last of the secure flags on the command line, matching the backend's BooleanOptionalAction."""
    if not not_secure:
        return secure
    last_secure = max(
...
def _iter_editable_studio_source_roots(venv_dir: Path):
    """Yield repo roots from setuptools `__editable___*_finder.py` files in *venv_dir*'s site-packages whose MAPPING includes a `studio` entry. Returns the parent dir of the mapped `studio` package, so callers can append `/studio/...` to reach any subdir."""
    import ast
    import re
        ...
        for sp in venv_dir.glob(sp_pattern):
            for finder in sp.glob("__editable___*_finder.py"):
                try:
                    ...
                    continue
                # Tolerate single- or multi-line dict literals; [^}]* still rejects nested dicts, which the setuptools template never emits for editable installs.
                ...
                try:
                    ...
                    continue
                # Defensive: literal_eval can return a set / list / None if the matched literal is not a dict (the regex captures `{...}`).
...
def _find_frontend_dist() -> Optional[Path]:
    """Locate a built `studio/frontend/dist` (containing index.html). Probes, in order: package-local default, installer venv site-packages, editable source roots referenced from the installer venv. None if nothing servable is found, so callers can decide to error or proceed in `--api-only` mode. Fixes the silent 404 when another `unsloth` on PATH shadows the installer's binary and points `_PACKAGE_ROOT` at a site-packages copy that never received a vite build."""
...
def _create_api_key_inprocess(name: str) -> str:
    """Create an API key via a direct storage call, bypassing the ``must_change_password`` gate that blocks HTTP ``POST /api/auth/api-keys`` on fresh installs. Safe because the CLI already has filesystem access to ``~/.unsloth/studio``."""
...
def _write_auth_secret(path: Path, secret: str) -> None:
    ...
    try:
        try:
            ...
            pass
        # newline pins LF: text mode writes CRLF on Windows, and `$(cat ...)` strips the LF but leaves the CR glued to the credential.
...
def _connect_auth_db() -> sqlite3.Connection:
    ...
    # Mirror backend storage.get_connection: this path can create auth/ and auth.db (the pre-exposure gate writes here first), and sqlite3.connect makes the DB 0644 under a 022 umask. Keep both private.
...
def _should_prompt_password_change(
    *, cloudflare: Optional[bool], host: str, secure: bool, api_only: bool
    ...
    """Whether this launch will expose Unsloth through the Cloudflare tunnel. CLI mirror of run.py's _cloudflare_tunnel_should_start, minus the Colab case (Colab launches never come through this CLI path). --secure implies the tunnel; --cloudflare only tunnels non-api-only wildcard binds."""
    if secure:
        return True
...
def _bootstrap_deadline_active() -> bool:
    """Whether the backend's bootstrap shutdown deadline will arm. Mirror of studio/backend/auth/bootstrap_timeout.py bootstrap_timeout_seconds: unset, blank or malformed UNSLOTH_STUDIO_BOOTSTRAP_TIMEOUT falls back to the 1h default, since a typo must not remove protection; 0 or negative disables it."""
...
def _cli_update_password(
    ...
    *,
    ...
    """CLI mirror of backend update_password and the change-password route effects. One transaction: rehash, rotate the JWT secret, clear must_change_password, revoke refresh tokens (PR #6651 finding), drop the desktop secret, and for a reset the API keys the old credential could have minted. File cleanup happens after commit; a failed unlink must not roll the change back."""
...
def _apply_supplied_password_before_launch(supplied_password: "str | None") -> None:
    """Non-interactively set the INITIAL admin password (from --password / UNSLOTH_STUDIO_PASSWORD / stdin) before the server binds, while the account still has its auto-generated bootstrap password. Only ever sets the FIRST password: an already-set one is a hard error, since an override would be an auth bypass on a public launch, and an invalid value fails closed. Runs in the parent before any re-exec so the secret never crosses to the child argv."""
    ...
    try:
        ...
        # Any DB failure fails closed (typer.Exit is not caught here, so the deliberate Exit(1) branches above propagate unchanged).
        typer.echo(
...
def _require_servable_frontend_or_exit(
    *, frontend: Optional[Path], api_only: bool, cloudflare: Optional[bool], host: str, secure: bool
    ...
    """Fail closed BEFORE the pre-exposure gate if a public UI launch has no login page to change the seeded password. The gate strips the seeded .bootstrap_password on a headless public launch, so if the child then cannot serve the login page the admin is locked out (must_change_password=1, no file, no UI) until `unsloth studio reset-password`. The login page is the ONLY in-band way to change the seeded password. Returns the dist to serve: a user-supplied --frontend (validated to contain index.html) or the auto-resolved built dist, and `frontend` unchanged for non-public or --api-only launches."""
    if api_only or not _should_prompt_password_change(
        cloudflare = cloudflare, host = host, secure = secure, api_only = api_only
    ):
        return frontend
    if frontend is not None:
        # A user-supplied dist is not vetted by _find_frontend_dist, so verify it can serve the login page; else `--frontend /bad/path` bypasses the guard.
...
def _tunnel_binary_confirmed_unavailable() -> bool:
    """True only if cloudflared is provably unavailable (found nowhere on PATH or in the Unsloth cache AND the download failed), so the tunnel cannot start. Used on the --secure path, where the bind is loopback and the tunnel is the ONLY public exposure, to skip stripping the seeded recovery password before a public URL that will never come up. Loads the stdlib-only cloudflare_tunnel helper by file path so the check runs in the parent, before the strip. Returns False on ANY uncertainty: a possible credential leak outweighs a recoverable lockout."""
    run_py = _find_run_py()
    if run_py is None:
        return False
    ...
    if not tunnel_py.is_file():
        return False
    # ensure_cloudflared() lazily imports utils.paths.storage_roots to resolve the Unsloth bin cache. The outer CLI has not added studio/backend to sys.path yet, so that import would fail and return None, a false "unavailable" that wrongly refuses --secure. Add the backend dir so the cache path resolves as in the child.
...
def _enforce_password_change_before_exposure(
    *,
    ...
    """Force a terminal password change before the first public (tunnel) exposure. When the launch will start the tunnel and the admin still has its auto-generated bootstrap password, ask for a new one in the terminal (masked, confirmed) before any server or tunnel exists. Committing here, in the parent, keeps the password off argv and env and an older studio-venv child sees it immediately. Without a terminal, warn and fall back to the bootstrap shutdown timer (~1h, UNSLOTH_STUDIO_BOOTSTRAP_TIMEOUT)."""
    if not _should_prompt_password_change(
        cloudflare = cloudflare, host = host, secure = secure, api_only = api_only
    ):
        return
    # Before public exposure we must PROVE the admin password is no longer the seeded default. If we cannot (auth DB will not open, or a fresh admin cannot be seeded and committed below), an old studio-venv child could regenerate a fresh bootstrap credential and serve it, and stripping a file we cannot vouch for cannot stop a regeneration. So those cases fail closed, as does a failure after the user typed a new password.
    try:
        conn = _connect_auth_db()
    except (OSError, sqlite3.Error) as exc:
        # Cannot open the auth DB, so cannot confirm a committed admin exists. Refuse rather than risk a child serving the default login; a transient lock clears on retry.
        typer.echo(
    ...
    try:
        try:
            _ensure_cli_default_admin(conn)
            # Persist a freshly seeded admin before we might re-exec: the INSERT is otherwise uncommitted and rolls back on conn.close(). If the seed or commit fails, no admin is committed, so a re-exec'd OLD child finds none, regenerates a fresh bootstrap password and file, and serves THAT, which stripping cannot stop. Cannot prove a committed admin, so fail closed.
            conn.commit()
        except (OSError, sqlite3.Error) as exc:
            # Best-effort remove any half-written seed file (its row rolled back); the launch is refused regardless.
            try:
        ...
        try:
            ...
            if child_self_suppresses:
                # Could not read must_change back, but the child is this install's own backend and suppresses the injection, so nothing serves the seeded credential; proceed without stripping.
                return
            # The admin is committed above, so an old child finds it and will not regenerate; we just could not read must_change back. Strip the seeded file so nothing serves it, failing closed if the strip itself fails.
            typer.echo(
        ...
        if not _prompt_streams_interactive():
            # Only proceed headless if the bootstrap shutdown deadline will protect the launch: it never arms for api-only, and TIMEOUT=0 disables it.
            if api_only or not _bootstrap_deadline_active():
                ...
                raise typer.Exit(1)
            if child_self_suppresses:
                # The child is this install's own backend, whose pre-bind gate sets app.state.suppress_bootstrap_injection, so the seeded credential is never served publicly even with the file on disk. Skip the strip: unnecessary here, and it would lock the user out if the tunnel never comes up. Keep the file for LOCAL recovery; must_change stays set and the deadline arms.
                typer.echo(
                ...
                return
            # The strip permanently removes the only plaintext recovery credential. On --secure the bind is loopback, so the tunnel is the ONLY public exposure: if cloudflared is provably unavailable no public URL can start and stripping would just lock the user out. A wildcard --cloudflare bind is public regardless of the tunnel, so it still strips below, as does any uncertainty.
...
def _load_model_via_http(
    ...
    try:
        ...
        # A slow load commits its 200 before it finishes and pads the body, so a late failure arrives in-band; raise it as the HTTPError this function already turns into the RuntimeError the caller reports. A truncated body is no report at all.
...
def studio_default(
    ...
    if ctx.invoked_subcommand is not None:
        # Typer does not forward parent options to subcommands, so `unsloth studio --parallel N run ...` would silently drop N.
    ...
    # --verbose restores the per-request access logs that are suppressed by default (plain-server path; the `run` subcommand has its own --verbose).
    if verbose:
    ...
    # Use the studio venv if present and not already in it. Resolve the child launcher BEFORE the gate: a headless gate strips the seeded .bootstrap_password, so aborting afterward would leave must_change_password=1 with no password to log in.
    studio_venv_dir = STUDIO_HOME / "unsloth_studio"
    in_studio_venv = sys.prefix.startswith(str(studio_venv_dir))
    # Before any of the three launch paths below, and before the environment is handed to a child: an override contradicting single-arch wheels makes every kernel launch fail, and the installer's own unset cannot reach a launch it does not perform (#7331).
    ...
    if not in_studio_venv:
        ...
        if not (studio_python and run_py):
            ...
            raise typer.Exit(1)
        # A public UI launch must have a servable login page BEFORE the gate can strip the seeded .bootstrap_password, or the child has no way to change it. Also returns the resolved dist so the child serves a real build regardless of where its __file__ lands.
        ...
        # Non-public / api-only launches skip that validation but still forward an explicitly resolved dist for the same silent-404 reason.
        if resolved_frontend is None and not api_only:
            resolved_frontend = _find_frontend_dist()
    else:
        # Already in the studio venv: no re-exec, served in-process below. On the headless public path the gate strips the seeded .bootstrap_password, so validate BOTH FIRST, else a bad dist or broken venv fails only after the strip. Frontend check first (cheap); the backend import is headless-only so an interactive prompt is not delayed behind it.
        resolved_frontend = _require_servable_frontend_or_exit(
            frontend = resolved_frontend,
            api_only = api_only,
            cloudflare = cloudflare,
    ...
    # A supplied --password / UNSLOTH_STUDIO_PASSWORD / stdin sets the initial admin password here in the parent, before the gate and any re-exec, so the secret never reaches the child argv; strip the env var so a re-exec'd child cannot re-read it. The interactive gate below then no-ops.
    ...
    # Public (tunnel) exposure with the seeded default password: force a terminal password change first, before any re-exec or server exists. The child is self-suppressing when we serve in-process or re-exec this install's own run.py, so the gate can skip the destructive strip.
    ...
    if not in_studio_venv:
        if studio_python and run_py:
            ...
            # Forward polarity explicitly: _find_run_py can fall back to an older run.py (--cloudflare defaulted on), so an unset default must not let a mixed install silently re-enable the tunnel. --secure implies it, so forward nothing then.
            ...
            if sys.platform == "win32":
                ...
                # Hand our std handles to the child: without them CREATE_NO_WINDOW gives the backend its own hidden console and `unsloth studio > log` captures nothing. Omitting stdin does not withhold it (subprocess still fills it from GetStdHandle); that would need stdin = DEVNULL.
    ...
    with _studio_runtime_launch_guard(
        ...
        # Forward the frontend validated before the gate (in-venv path), so the in-process server serves exactly the dist we vouched for.
        if resolved_frontend is not None:
    ...
    try:
        if run_mod._shutdown_event is not None:
            # Event.wait() with no timeout blocks at C-level on Linux and swallows SIGINT; loop with a 1s timeout instead.
...
def _split_repo_variant(model_arg: str) -> tuple[str, Optional[str]]:
    """Split ``org/name:variant`` into ``(repo, variant)``, mirroring llama.cpp's ``-hf <repo>:<quant>``. Local paths, Windows drives and ids without ``:`` pass through verbatim."""
    s = model_arg.strip()
    if not s:
        return s, None
...
def _expand_attached_np_short() -> None:
    # Click clusters `-np8` as `-n -p 8` (-p = --port), dropping the parallel value. Split to `-np <N>` so typer's alias matches. Stops at `--`; accepts signed/junk forms so typer reports a clean error against `-np`. Kept in lockstep with the backend `_flag_name` recogniser.
    i = 0
...
def _consume_legacy_short_aliases(
    ...
    """Pop exact-match legacy shorts (`-m`/`-hfr`/`-f`) from args; leave clusters (`-mg`/`-fa`/...) for the llama-server tail. The inline `-x=value` form is also accepted."""
    out: List[str] = []
    value = current
    i, n = 0, len(args)
    while i < n:
...
def run(
    ...
    model: Optional[str] = typer.Option(
        ...
        "-hf",
        "--hf-repo",
        # `-m` / `-hfr` removed, since Click would cluster `-mg`/`-md`/...; exact-match `-m`/`-hfr` still work via the legacy shim below. `-hf` stays, as multi-char shorts do not cluster.
    ...
    """
    # A newer outer CLI can re-exec into an older Unsloth venv; pass this signal via env so an older child ignores it instead of treating it as a llama-server arg.
    ...
    runtime_gate_handoff = _studio_runtime_gate.consume_runtime_gate_handoff()
    # The group callback returns before its own clear once a subcommand is named, and `unsloth run` is bound straight here, so this path has to do it itself or llama-server and the backend start with the contradicting override (#7331).
    ...
    # Tool-call healing and nudging are read from the env at backend import. Resolve here, before any re-exec or import, so the in-venv child inherits the decision. When the flag is omitted, respect a value the parent already set and otherwise apply the default: healing on, nudging on for a CLI-launched server.
    _healing_disabled = (
    ...
    # Sampling overrides: the backend resolver reads UNSLOTH_SAMPLING_* to hard-pin a field, winning over both the client and the per-model recommendation. Only write a flag that was set explicitly so an omitted flag inherits any value the parent forwarded and, when nothing is set, leaves the per-model recommendation in charge.
    ...
    # Set before any re-exec so the in-venv server inherits it via the env. `run --verbose` used to pass through to llama-server (its own -v); keep that by forwarding --log-verbose so we add Unsloth logs without dropping it.
    if verbose:
        _enable_verbose_access_logs()
    ...
    # Parse llama.cpp `repo:variant` in --model; error if also paired with --gguf-variant and they disagree.
    ...
    # Tool policy does not depend on the bind: tools default on everywhere (--secure is a loopback tunnel; the operator owns a raw bind). With no flag this stays None, so the default applies without becoming an override and a request's own enable_tools: false is honored. Resolve here so the re-exec'd child inherits the same decision.
    ...
    # Re-exec into the studio venv, as studio_default does. Resolve the child launcher BEFORE the gate: a headless gate strips the seeded .bootstrap_password, so aborting afterward would leave must_change_password=1 with no password to log in.
    studio_venv_dir = STUDIO_HOME / "unsloth_studio"
    in_studio_venv = sys.prefix.startswith(str(studio_venv_dir))
    studio_bin = None
    resolved_frontend = frontend
    if not in_studio_venv:
        ...
        if not studio_python:
            typer.echo("Unsloth Studio not set up. Run install.sh first.")
            raise typer.Exit(1)
        # Re-exec via the studio venv's `unsloth` console script. Windows ships it as unsloth.exe, so the bare name is never a file there and `unsloth run` aborted with "venv missing 'unsloth' entry point" on a perfectly good install. On Windows the file is no longer what gets launched and no longer the only thing that proves a CLI: quarantine deletes the stub and leaves the environment able to run, so the installed package answers for it.
        ...
        if not studio_bin.is_file() and not _managed_cli_package_present(studio_python):
            ...
            raise typer.Exit(1)
        # `run` serves the same Unsloth UI unless --api-only, so a public launch must have a servable login page BEFORE the gate strips the seeded password. Validate here and forward the resolved dist so a shadowed child that cannot self-resolve one still serves it.
        resolved_frontend = _require_servable_frontend_or_exit(
            frontend = frontend,
            api_only = api_only,
            cloudflare = cloudflare,
        ...
        # In-venv (in-process) run: validate the servable frontend and importable backend before the headless gate strips the seeded password. Frontend check first (cheap); the backend import is headless-only so a prompt is not delayed.
        resolved_frontend = _require_servable_frontend_or_exit(
            frontend = frontend,
            api_only = api_only,
            cloudflare = cloudflare,
    ...
    # A supplied --password / UNSLOTH_STUDIO_PASSWORD / stdin sets the initial admin password here in the parent, before the gate and any re-exec, so the secret never reaches the child argv; strip the env var so a re-exec'd child cannot re-read it.
    _apply_supplied_password_before_launch(_password_prompt.resolve_supplied_password(password))
    os.environ.pop(_password_prompt.SUPPLIED_PASSWORD_ENV, None)

    # Public (tunnel) exposure with the seeded default password: force a terminal password change first, before any re-exec or server exists. The re-exec here runs the studio venv's `unsloth` console script, a possibly-OLD child, so it is NOT provably self-suppressing; only the in-process case is, and the strip stays in force otherwise.
    _enforce_password_change_before_exposure(
        cloudflare = cloudflare,
        host = host,
        secure = secure,
    ...
    if not in_studio_venv:
        # Windows launches the child through the venv interpreter rather than the console script it just validated: Application Control blocks the generated unsloth.exe on some machines, and the signed python.exe beside it is not blocked. POSIX keeps execing the script directly, which is what the os.execvp below needs anyway.
        launch_head = (
        ...
        # Forward the explicit polarity; a future default flip on one layer must not silently invert behaviour for the other.
        args.append("--load-in-4bit" if load_in_4bit else "--no-load-in-4bit")
        # Forward the frontend resolved before the gate, not just a user-supplied one: the parent may have found a built dist the shadowed child cannot, and stripping without forwarding it would abort the child at frontend setup (lockout).
        if resolved_frontend is not None:
            args.extend(["--frontend", str(resolved_frontend)])
        if api_only:
            args.append("--api-only")
        if silent:
            args.append("--silent")
        # Forward the resolved tool policy so the child does not re-resolve. None forwards neither flag: the child then leaves the policy unset too.
        if enable_tools is True:
            args.append("--enable-tools")
        elif enable_tools is False:
            args.append("--disable-tools")
        ...
        if yes:
            args.append("--yes")
        # Typer claims --parallel outside ctx.args; without this the child reverts to its default and silently drops the value.
        args.extend(["--parallel", str(parallel)])
        # Always forward explicit polarity: a mixed-version studio venv whose old default was --cloudflare-on must not silently re-enable the tunnel. --secure implies it, so forward nothing then.
        if cloudflare is True:
            args.append("--cloudflare")
        elif not secure:
            args.append("--no-cloudflare")
    ...
    # Match the route handlers' import path: run.py adds studio/backend/ to sys.path, so they import as `state.tool_policy`. Set this before run_server() starts uvicorn; once sockets are bound, routes can be hit. run_server() applies the same pair; both calls are idempotent.
    ...
    run_kwargs = dict(
        ...
        cloudflare = cloudflare,
        secure = secure,
        # Headless serving prints its own URL/API-key banner; the Tauri-only TAURI_PORT line would corrupt that machine-parseable output.
        emit_tauri_port = False,
        # We read the bound port back below, so a fallback past another Unsloth is safe here and keeps side-by-side model runs working.
    ...
    # Steps 3-5 can abort (health timeout, model-load error, or Ctrl+C during the slow load); tear the server and its children (llama-server, cloudflared) down on any abort so they never orphan.
    ...
    try:
        ...
        if start_api_key_marker:
            # `unsloth start` reads this key from a private 0600 log to authenticate download-progress polling; the normal `unsloth run` output is unchanged.
    ...
    # 6. Print banner. Keep the public host for reachability, but print a LAN or loopback URL.
...
def _pid_alive(pid: int) -> bool:
    """Return True if a process with ``pid`` exists. ``os.kill(pid, 0)`` raises OSError (WinError 87) for every pid on Windows, so use ``tasklist`` there and the signal-0 probe elsewhere."""
    if sys.platform == "win32":
        try:
            out = subprocess.run(
...
def _unlink_quietly(path: Path) -> None:
    """Drop a record without letting one bad file end the loop: an undeletable record must not stop us reaching the other servers, which is the orphan this command exists to prevent."""
    try:
...
def _pid_file_entries(
    ...
    """(pid, create_times, files) per recorded server, including the legacy studio.pid. Paths that could not be read are appended to `unreadable` when given, so the caller can tell "nothing is running" apart from "something is running and we could not see it". Grouped by PID: a server writes both its per-port file and studio.pid, and signalling twice would hit the SIG_DFL the first SIGTERM installs, hard-killing it mid-shutdown. Every recorded time is kept, since a stale file and a live server can share a PID and the stale one must not veto the live one."""
    ...
    for path in paths:
        ...
        try:
            ...
            # Unreadable is not the same as invalid. A root-owned record, or one caught mid-write, still belongs to a live server, and deleting it strands that server, the bug this command exists to fix.
...
@studio_app.command()
def stop():
    """Stop every running Unsloth Studio server for this STUDIO_HOME. The port fallback can leave more than one running, so stop them all."""
    unreadable: "list[Path]" = []
    entries = _pid_file_entries(unreadable)
    if not entries:
        if unreadable:
            # Reporting success here would be a lie: the records we could not read are kept, and the servers behind them are still serving.
...
def _wait_for_windows_setup_process(process) -> int:
    ...
    try:
        ...
        try:
            ...
            # taskkill interrupted or unavailable: hold the gate until setup exits naturally rather than exposing a live mutator.
            pass
...
# -NoProfile below drops $PSDefaultParameterValues wholesale, and on a corporate host a profile entry such as 'Invoke-WebRequest:Proxy' may be the only route to the VC++ runtime and the uv installer setup.ps1 downloads. install.ps1 hands over the keys it kept; a standalone update has no installer above it, so the probe below asks a short-lived PowerShell that DOES load the profile.
...
_PS_PROXY_PROBE = (
    ...
    # Windows PowerShell 5.1 writes REDIRECTED output in the console code page while this process decodes UTF-8, so a non-ASCII proxy value came back with replacement characters, still parsed as JSON, and handed setup a proxy that does not work.
    ...
    # Launched -NoProfile, with the caller's profiles dot-sourced by name instead, because left to itself this process loads the CONSOLEHOST profile, an unrelated host's for a caller in the VS Code console. $PROFILE is fully populated under -NoProfile (the paths are computed, not loaded), so naming them is exact.
    "$__unslothHostProfileName = $env:_UNSLOTH_PS_HOST_PROFILE; "
    # All-users first, PowerShell's own startup order: a machine-managed proxy lives in AllUsersAllHosts on a domain-joined box, and the user's profile only overrides it by running last.
    "try { $__unslothProfiles = @($PROFILE.AllUsersAllHosts); "
    # ONLY the caller's host profile, and only when the caller could be identified: sourcing every Microsoft.*_profile.ps1 in the directory ran profiles for hosts nobody was using, which can print, clobber $PSDefaultParameterValues, or exit before the record is written.
    ...
    # ADDED, never substituted: TERM_PROGRAM=vscode is set by every VS Code integrated terminal, not only the PowerShell extension's own host, so a plain pwsh terminal there loads Microsoft.PowerShell_profile.ps1 and substitution missed the proxy it actually has.
    "if ($__unslothHostProfileName) { "
    ...
    # Re-pinned, because setting [Console]::OutputEncoding is an ordinary profile customization that overrides the pin above. The parent decodes this stream as UTF-8, so a console left on a legacy code page corrupts the framed record and the proxy URI with it.
    "try { [Console]::OutputEncoding = "
    "New-Object System.Text.UTF8Encoding $false } catch { }; "
    "try { $OutputEncoding = [Console]::OutputEncoding } catch { }; "
    "$out = @{}; "
    ...
    # A script block is the supported form for a DYNAMIC default, e.g. { [uri]$env:CORP_PROXY }, evaluated per call by Invoke-WebRequest. Evaluate here and serialize the RESULT: executable code must not cross the handoff.
    "elseif ($v -is [scriptblock]) { try { $r = & $v; "
    "if ($r -is [uri]) { $out[$k] = $r.AbsoluteUri } "
    "elseif ($r -is [string] -or $r -is [bool]) { $out[$k] = $r } } catch { } } } }; "
    # $out already holds copies, so the profile's table is now only a hazard: ConvertTo-Json:AsArray = $true is a legitimate setting that turns this record into a JSON array, which the reader rejects for not being a dictionary.
    "$PSDefaultParameterValues = @{}; "
    # FRAMED, not bare: the profile is free to print a banner or a MOTD, and that output arriving ahead of the JSON made the parse throw. Module-qualified, as with the uv lookup: an alias named ConvertTo-Json or Write-Output would otherwise reshape the frame.
    f"if ($out.Count -gt 0) {{ "
...
# What the CALLER's host names its own CurrentUserCurrentHost profile, when we can tell. An unidentifiable host gets no extra profile rather than someone else's.
...
def _windows_powershell_module_path(current: str) -> Optional[str]:
    """``current`` with Windows PowerShell's own module directory first, or None to leave it be. PowerShell 7 strips its module paths only when IT launches powershell.exe, so reached through this Python process the child keeps them in FRONT and resolves 5.1's own modules to the PowerShell 7 copies; a profile that imports one then throws while it is dot-sourced and the proxy it was going to publish is lost. Same repair as install.ps1, applied per probed host: pwsh needs none, because PowerShell 7 prefixes its own paths on startup."""
...
def _profile_probe_env(host: str = "") -> dict:
    """The probe child's environment: the caller's host profile when it is known, and the probed host's own module precedence."""
    env = dict(os.environ)
    ...
    if platform.system() == "Windows" and _fold_module_entry(host).rsplit("\\", 1)[-1] in (
        ...
        # os.environ upper-cases keys on Windows, so the copy is keyed PSMODULEPATH: reading "PSModulePath" misses the caller's value and adds a second, case-differing entry.
...
# Annotations below are quoted: this module has no `from __future__ import annotations`, and on Python 3.9 evaluating `str | list[str]` at def time raises TypeError, taking the CLI import with it.
...
def _probe_profile_proxy_defaults(powershell: "str | list[str]") -> Optional[str]:
    """The caller's profile proxy defaults as JSON, or None. install.ps1 hands these over in the environment, but a standalone `unsloth studio update` has no installer above it and a PowerShell variable does not reach this Python process, so ask for it in a throwaway process whose only job is to print the table. Several hosts may be given: their answers are MERGED, earlier hosts winning per key. Best effort, since a slow, interactive or broken profile costs one timeout and the child proceeds as it does today."""
    hosts = [powershell] if isinstance(powershell, str) else list(powershell)
    # ONE budget for the whole probe, not one per host: with both editions installed, two hung profiles would otherwise stall every standalone update for twice the stated cost.
    deadline = time.monotonic() + _PROFILE_PROBE_TIMEOUT_SECONDS
    merged: dict = {}
    # $PSDefaultParameterValues keys are case-INSENSITIVE, so "Invoke-WebRequest:Proxy" and "invoke-webrequest:proxy" are one entry to PowerShell and two to a Python dict. Only the first spelling seen is handed on, or the prelude would replay both and let the lower-priority host's land last.
    claimed: dict = {}
    seen_keys: set = set()
    for host in hosts:
        ...
        try:
            probe = subprocess.run(
                ...
                capture_output = True,
                text = True,
                # text=True alone decodes with the locale codec and STRICT errors, so a UTF-8 profile banner on an ANSI console raised UnicodeDecodeError, neither OSError nor SubprocessError, so it escaped the handler below and took the update down.
                encoding = "utf-8",
                errors = "replace",
                timeout = remaining,
                **_windows_hidden_subprocess_kwargs(),
        ...
        if not payload:
            continue
        try:
            # Validated, not trusted: the framing finds the record, this confirms it is one, so nothing hands the child a string ConvertFrom-Json throws on.
            parsed = json.loads(payload)
        except ValueError:
            continue
        if not isinstance(parsed, dict) or not parsed:
            continue
        # Per CMDLET, not per key: pairing the earlier host's Proxy with the later host's ProxyUseDefaultCredentials would offer the user's Windows credentials to a proxy whose own profile never asked for that. A cmdlet is claimed whole by the first host.
...
def _cmdlet_claimed_elsewhere(cmdlet: str, claimed: dict, source: object) -> bool:
    """Whether another host already owns the cmdlet family ``cmdlet`` belongs to. A literal comparison is not enough: the command half of a $PSDefaultParameterValues key may be a wildcard, and PowerShell applies such an entry to every cmdlet it matches, so overlap in either direction claims the family."""
...
def _patterns_can_overlap(left: str, right: str) -> bool:
    """Whether any command name matches BOTH wildcard patterns. Two patterns can share matches without either matching the other as a STRING (Invoke-Web* and *-WebRequest both apply to Invoke-WebRequest), so the languages are intersected rather than string-matched, or a host's Start-Bits* entry swallowed the other host's unrelated Invoke-Web* one. A character class is not decided, only assumed to overlap: that is the conservative answer, and no cmdlet name is written that way."""
    if "[" in left or "[" in right:
        return True

    @functools.lru_cache(maxsize = None)
    def walk(i: int, j: int) -> bool:
        # Both patterns are consumed in step, except at a '*', which may absorb one more character from the other side or match nothing at all.
        if i == len(left):
...
_PS_PROXY_DEFAULTS_PRELUDE = (
    ...
    # Read once, then GONE from the environment: a profile proxy can carry credentials (http://user:secret@proxy is the ordinary corporate form) and every native process setup starts would otherwise inherit it. Needed only by this session's $PSDefaultParameterValues, which is not inherited.
...
def _uv_cache_has_packages(cache_dir: Path) -> bool:
    """wheels-* is metadata only on uv 0.10, so counting any file reads a cache that was merely resolved against as warm. Same rule as install.sh:_configure_uv_cache."""
    try:
        buckets = [
            entry
        ...
        # ValueError too: an embedded NUL builds a Path fine and then raises out of scandir, aborting an update over a file that is only advisory.
        return False
...
# uv's boolish spelling. Anything outside it is a value uv refuses to run on, which is not "no cache" either.
_UV_TRUE = ("1", "true", "yes", "on")


def _uv_no_cache_requested() -> bool:
    """uv --no-cache caches in a temporary directory and discards it on exit, so the probe would report that throwaway, --no-cache outranks --cache-dir anyway, and recording it would aim later updates at a cache that never existed."""
...
def _uv_default_cache_dir(cwd: Optional[Path] = None) -> Optional[Path]:
    """Asked of uv, not reconstructed, so uv.toml and UV_CONFIG_FILE count. Asked from where setup will ask it, too: both setup scripts change into their own directory before the dependency pass (studio/setup.sh:1788), and uv discovers uv.toml and pyproject.toml from its working directory, so probing in the caller's would answer for whatever project the user happens to be standing in."""
    uv = shutil.which("uv")
    if not uv:
    ...
    if not lines:
        return _uv_platform_cache_dir()
    # uv answers a relative cache-dir with the relative spelling, against its own working directory, and resolves a relative UV_WORKING_DIR after starting where this probe did. No expanduser: uv makes a literal "~" directory, not one in $HOME.
...
def _recorded_install_uv_cache() -> Optional[Path]:
    """The cache the installer used, as it recorded it. Content cannot tell a Studio cache the installer filled from one holding a single wheel the running backend dropped there (wheel_utils.py:405), since install.sh:705 points it there even in shared mode. Absent, the caller falls back to content."""
    try:
        # utf-8-sig: Windows PowerShell 5.1 writes `-Encoding utf8` WITH a BOM. surrogateescape: a POSIX path may not be UTF-8, and U+FFFD would name nothing.
        recorded = (STUDIO_HOME / "cache" / "uv-cache-dir").read_text(
            encoding = "utf-8-sig", errors = "surrogateescape"
        )
    except OSError:
        return None
    # One record, one trailing delimiter, everything before it the path: splitting on lines would take a POSIX path containing a newline for several. Not otherwise stripped, since a path may end or begin with a space; blank means unset.
...
def _backfill_uv_cache_marker(env: Optional[dict]) -> None:
    """Record the cache this update used, for installs whose installer never did. Those reach the content fallback, which goes stale the moment the backend drops one wheel into the Studio cache and both caches look warm; writing the choice down once setup has succeeded closes that, and likewise for a marker whose cache was emptied."""
    ...
    if stage_root:
        # STUDIO_HOME names the LIVE install here and the stage can still be rejected, so the choice is parked and _studio_stage.stage promotes it on acceptance. Dropping it instead left desktop-only installs on the content fallback.
    ...
    try:
        ...
        # fsencode: an undecodable path arrives as surrogates, and encoding those raises UnicodeEncodeError, which is not an OSError.
...
def _with_studio_uv_cache(env: Optional[dict], cwd: Optional[Path] = None) -> Optional[dict]:
    """An update reached neither installer nor _setup_cache_env, so uv re-downloaded what the install had just fetched."""
    if (os.environ.get("UV_CACHE_DIR") or "").strip():
        return env
    if _uv_no_cache_requested():
    ...
    # No marker, so this install predates it and content is all there is, and content cannot settle it: install.sh:705 points the running backend at the Studio cache even in shared mode, so one on-demand wheel warms it. uv's default is what such an install has been updating from all along, and it cannot record its way out of a wrong guess.
    default_cache = _uv_default_cache_dir(cwd)
    if default_cache is not None and _uv_cache_has_packages(default_cache):
        # Named, not re-resolved: a blank inherited value reaches uv as `--cache-dir ''` and exits 2.
...
def _run_setup_script(*, verbose: bool = False, repo_root: Optional[Path] = None) -> None:
    ...
    # Where setup will run uv from, which differs by platform: setup.sh changes into its own directory (setup.sh:1788), setup.ps1 never does and hands install_python_stack.py the cwd it inherited from here (setup.ps1:5191).
    setup_cwd = None if platform.system() == "Windows" else script.parent
    env = _with_studio_uv_cache(env, cwd = setup_cwd)

    if platform.system() == "Windows":
        # Resolved, not bare: the gate that runs immediately before this in setup() and update() had to stop trusting PATH for exactly this reason (#9440), and the Popen below has no OSError handler, so a bare name here just moves the same WinError 2 one frame later.
        powershell = _studio_runtime_gate.resolve_windows_powershell()
        powershell_args = [powershell]
        # PRESENCE, not truthiness: install.ps1 publishes this around the handoff and sets it to "{}" when it found no proxy, so treating that as "nobody handed anything over" would send an installer launch off to reload the profiles it deliberately discarded.
        ...
        # -NoProfile unconditionally, not just on the hidden branch: install.ps1 hands off to exactly here from a console where stdout is a tty, so the hidden branch does not fire and a profile that aliases uv or python would break setup.ps1.
        ...
        # Use -Command + `*>&1` (not -File) so setup.ps1's Write-Host output (Information stream #6) merges into stdout; -File drops it when stdout is a pipe, e.g. `unsloth studio update --local 2>&1 | tee`. Single-quote escaping handles paths containing apostrophes.
        ...
        # Explicitly hand std handles to the child so CI tee sees setup.ps1's output. On Windows, subprocess.Popen defaults to close_fds=True (bInheritHandles=False); combined with CREATE_NO_WINDOW the child has no console and no inherited handles, so Write-Host writes to nothing. Passing stdout/stderr makes Python mark the std handles inheritable via PROC_THREAD_ATTRIBUTE_HANDLE_LIST. Empty update.log on windows-latest CI was the smoking gun (runs 25533694490/25534292239).
...
# The refresh re-runs the installer with --shortcuts-only, fetched rather than shipped so a launcher fix reaches users without waiting for a release.
_INSTALLER_URL_BASH = "https://unsloth.ai/install.sh"
_INSTALLER_URL_PWSH = "https://unsloth.ai/install.ps1"
# unsloth.ai 301s to raw.githubusercontent.com, so both are in the chain. Anywhere else, or plain http, is refused rather than followed.
...
# The flag this code passes, so an installer without it cannot serve the request. Internal names would be tighter but can be renamed in a perfectly good installer, and a false negative here skips every wheel-based refresh until new Python ships.
_INSTALLER_MARKERS = {
...
def _build_installer_opener() -> urllib.request.OpenerDirector:
    """A private opener, so the redirect chain can be checked before it is followed. urlopen() would instead use whatever install_opener() put in place. Carrying those handlers over was tried and abandoned: OpenerDirector.add_handler() assigns handler.parent, so sharing them repoints the installed opener at this one and breaks every later urlopen() in the process, and copying them does not help either because the default HTTPSHandler here already answers first. Proxy environment variables and the system trust store still work, since build_opener() sets up both. The residue is a machine whose proxy auth or CA lives in a programmatically installed opener: its refresh is skipped, which the update survives, rather than silently pulling the installer through an unchecked path."""
...
def _looks_like_installer(body: Optional[bytes], installer_name: str) -> bool:
    """Cheap shape check before a fetched installer is executed. Not a trust check, since the hosts above are trusted: it stops a captive-portal page or an HTTP error body from being piped into bash when something in between answers."""
    if not body or len(body) < 512:
        return False
    head = body.lstrip()[:256].lower()
    # `<#` opens PowerShell comment-based help, which is a perfectly ordinary way for install.ps1 to start, so match the actual markup rather than any leading "<".
...
def _fetch_installer(installer_name: str, *, verbose: bool = False) -> Optional[bytes]:
    ...
    try:
        ...
        with opener.open(request, timeout = _INSTALLER_FETCH_TIMEOUT) as response:
            ...
            # read(amt) does not check Content-Length; only a further read() does, raising IncompleteRead. Without it a transfer cut off mid-file still carries the markers and would be piped into bash half-written.
            if len(body) <= _INSTALLER_MAX_BYTES:
                body += response.read()
    except (
        urllib.error.URLError,
        # IncompleteRead / a malformed proxy response raises at the HTTP framing layer, which is neither URLError nor OSError, so it would abort an already-done update.
...
def _installer_script_candidates(installer_name: str) -> List[Path]:
    """Source-tree installers, which outrank the network because `update --local` is testing its own installer, so fetching over the top of it would be wrong."""
...
def _installers_on_disk(candidates: Sequence[Path]) -> List[Path]:
    """Every candidate that exists, not just the first. The pre-refactor loop probed and launched in one pass, so a candidate that could not be launched left the next one to try before the network was reached, and returning only the first match would quietly drop that second chance."""
    found: List[Path] = []
...
def _refresh_desktop_shortcuts(*, verbose: bool = False) -> None:
    ...
    if is_windows:
        ...
        # -NoProfile unconditionally, as in _run_setup_script above: gating it on the hidden branch left the visible console path, where a profile is exactly what IS loaded.
...
def _run_installer_bash(script: Path, args: Sequence[str], env: dict) -> bool:
    """False when the interpreter could not be launched, so the caller can fall back. The pre-refactor candidate loop caught that OSError and carried on to the next candidate and then to the network, silently; returning False keeps a machine that cannot spawn bash for the checkout on exactly that path instead of ending the refresh early."""
    try:
...
def _run_fetched_installer_ps1(
    ...
    """Run a fetched install.ps1 from a tempfile. -File rather than `-Command -`: stdin is decoded with [Console]::InputEncoding (CP1252/OEM on most Windows boxes), which mangles install.ps1's box-drawing chars, while -File honours the BOM written below. The args go after the path so the installer's own `Install-UnslothStudio @args` at EOF receives them, which is why this no longer rewrites that line, and the prefix gives AV/EDR engines (and anyone grepping temp) a clear identity. Creating and writing that file is its own failure mode (a full disk, a read-only or missing %TEMP%, or AV holding the handle): those raise OSError, and the refresh runs after the package update has already succeeded, so they are reported and skipped rather than allowed to abort the command."""
    try:
...
def setup(
    ...
    with _studio_runtime_launch_guard(inherited = runtime_gate_handoff):
        ...
        # Duplicate-metadata repair can reinstall unsloth even when the installer set SKIP_STUDIO_BASE. Free and preserve the running Windows launcher exactly as the direct update path does.
...
def _fail_if_install_damaged(package_name: str = "unsloth") -> None:
    """Refuse to call an update successful when the tree it produced is damaged. pip considers a distribution with intact metadata already satisfied, so an update reinstalls nothing when a package's FILES are damaged: it prints "Unsloth Studio Installed", exits 0, and the backend then dies at boot. That is the shape behind "just re-run the installer", and it is only actionable if the update says so."""
    ...
    if _studio_deps.running_outside_managed_venv((managed_venv,)):
        # This CLI does not live in the venv the update just wrote, so its own file list describes the wrong tree. Silence beats a wrong answer.
        return
    ...
    # Carry a custom root into the command: the shim is a bare symlink and _ensure_studio_env_exported only sets os.environ for this process, so the shell that runs this line has no UNSLOTH_STUDIO_HOME and an unqualified reinstall would build a fresh ~/.unsloth/studio and leave the damaged install exactly as broken as it was.
    # And carry the recorded install mode: install.sh derives SKIP_TORCH only from its own flag or UNSLOTH_NO_TORCH and passes that value into setup, so a plain reinstall over a GGUF-only install downloads the whole PyTorch stack. Only added when the record says True, since recorded_no_torch() returns None when nothing recorded the mode and None must never be read as False.
    # No root argument: the manifest and marker live in the VENV, not the install root, and recorded_no_torch defaults to Path(sys.prefix), so passing STUDIO_HOME would look one directory too high, find nothing, and silently never fire. The early return above guarantees sys.prefix is that venv.
    no_torch = False
    try:
    ...
    if platform.system() == "Windows":
        ...
        # The assignments go before `sh`, not before `curl`: that is the form install.sh documents, and it is sh that reads them.
        env = ""
        if _STUDIO_HOME_IS_CUSTOM:
    ...
    typer.echo("", err = True)
    # The installer installs the current requirement sets; it does not prune or reinstall anything outside them, so a package left over from an older release, or added by hand, is not repaired by the command above and would otherwise report the same damage forever. Say what to do in that case rather than scoping the scan, which would risk passing over real damage.
    ...
    # --no-deps: without it pip resolves the damaged package's dependency graph and --force-reinstall would replace pinned runtime packages too, which can swap the installed CUDA/ROCm torch build for a default one while repairing an unrelated orphan; the installer's own targeted repairs pair the two for the same reason. The interpreter path is quoted because a custom root may contain spaces, and on Windows a quoted command needs the call operator. <package>==<version> rather than a bare name: --force-reinstall reinstalls even when the package is already up to date, so an unpinned name fetches the newest release and silently upgrades an orphan whose consumers may depend on the older one, which --no-deps does not protect against.
...
def update(
    ...
    # Re-export UNSLOTH_STUDIO_HOME for env-mode installs so the refresh subprocess resolves the same install root the user originally chose.
    _ensure_studio_env_exported()
    # `is True`, not truthiness: only the CLI resolves the parameter to a bool. An in-process caller that leaves it out gets typer's OptionInfo sentinel, which is truthy, and every such call would stage instead of updating.
    if stage is True:
    ...
    if local:
        ...
        # Pass the repo root explicitly so install_python_stack.py does not have to guess from SCRIPT_DIR (which may be inside site-packages). Deriving it from __file__ only holds while this CLI runs from a checkout: once an update has installed unsloth into the venv non-editably, parents[2] IS site-packages, and uv rejects it with "does not appear to be a Python project: neither 'setup.py' nor 'pyproject.toml' found", which is what a second `update --local` hit on Windows, where the first update replaces the editable install. Absolutise the override, because setup.sh does `cd "$SCRIPT_DIR"` before it runs install_python_stack.py, so a relative STUDIO_LOCAL_REPO would be re-resolved against studio/ (no pyproject.toml) and hand uv back the very error this guard exists to replace. .strip()/.expanduser() match the handling in _refresh_desktop_shortcuts.
        ...
        if not (repo_root / "pyproject.toml").is_file():
            ...
            if platform.system() == "Windows":
                # PowerShell has no `VAR=value command` prefix form: it parses the assignment as a command name and fails to find it. This guard fires on the Windows update path, so the POSIX spelling would be unusable for most of the people who see it.
                typer.echo(
    ...
    # main gained a runtime gate around setup; this branch replaced the rename-to-.deleteme helpers with the launcher transaction. Both apply: the gate keeps a second Unsloth process off the venv, the transaction keeps the launcher recoverable across the setup it wraps.
    runtime_gate_handoff = _studio_runtime_gate.consume_runtime_gate_handoff()
    with _studio_runtime_launch_guard(inherited = runtime_gate_handoff or staging):
        if not staging:
            _studio_runtime_gate.ensure_managed_environment_is_idle(STUDIO_HOME)
        # Constructed after the idle scan, which test_studio_runtime_gate pins: the transaction wraps the mutation, so nothing of it may precede the gate.
        ...
        with launcher_transaction as launcher_update:
            ...
            # This deliberately runs even with --no-verify: the broad package scan is optional, but a successful update must leave its own launcher usable.
            launcher_update.validate_launcher()
            if verify:
                _fail_if_install_damaged(package)
    # Tauri desktop owns its own bundle entries; skip the CLI launcher refresh so a Tauri-initiated update does not create duplicate shortcuts.
...
class _WindowsLauncherUpdateTransaction:
    ...
    # Sentinel rather than a message: _launcher_health_error matches on identity, so it can never be confused with a real diagnostic that happens to read the same way, and it never reaches a user.
    _POLICY_BLOCKED = "an Application Control policy blocked the launcher"
    # Absence is not corruption. Quarantine takes the unsigned stub and leaves the environment intact, and nothing executes the stub any more, so the CLI can be perfectly healthy without it. Kept apart from the PE-shape failure, which is still a real one.
    ...
    def _recover_missing_launcher(self) -> None:
        ...
        # Validity, not existence: a truncated or quarantined launcher is just as unusable, and the backup beside it can repair either.
        ...
        for recovery in (self.backup, self.stale, self.legacy_backup, self.shim):
            if recovery is not None and self._is_valid_pe(recovery):
                try:
                    ...
                    # Try the next copy. The header check and the copy open the file separately, so antivirus taking a candidate in between is a race this loop can lose without the others being unusable, and giving up on the first one turned a recoverable install into a failure.
    ...
    def _move_launcher_aside(self) -> None:
        """Free the canonical path so the installer can publish a replacement. uv only self-replaces its OWN executable, so it deletes a third-party console script outright and hard-errors when the file is in use, after which the pip fallback no-ops on the already-satisfied bare unsloth and the upgrade is silently skipped. Renaming a running image is allowed on Windows: verified on windows-latest that renaming a live console-script launcher succeeds and a replacement can then be written at the freed path. Non-fatal, since failing to move it aside costs only the upgrade."""
        ...
        try:
            ...
            # Not fatal: an antivirus hold must not make the environment unupdatable. But say what it costs, because uv cannot then replace the launcher and the pip fallback drops --upgrade-package, so unsloth is left at its old version while everything else updates.
    ...
    def _recovery_candidates(self) -> List[Path]:
        """Copies that could stand in for the launcher, best first: the backup is the last launcher known to run, the moved-aside copy is only this run's unvalidated canonical file, and the legacy .deleteme and the PATH shim are what an install broken by the old updater still has. All of them are kept, because passing the two-byte header check does not make any one of them runnable and the next candidate has to be reachable."""
        seen: List[Path] = []
    ...
    def _restore_from(self, source: Path) -> bool:
        assert self.launcher is not None
        # The common setup-failure case leaves the original executable exactly where it was. Avoid replacing that running file on Windows when it already is the source byte-for-byte.
    ...
    def _restore_runnable(self) -> bool:
        """Put back the first copy that actually runs. Passing the two-byte header check does not make a copy runnable, so a candidate that fails --version must not stop the next one being tried, and a launcher already in place and working must not be replaced by a candidate that is merely PE-shaped. Under Application Control that discrimination degrades to the PE-shape check, because no candidate can be told apart by running it: every --version attempt dies in CreateProcess. That is a real limitation and not a hidden one, since a PE-shaped but corrupt copy can be left in place, but it is unavoidable, the only way to tell a good stub from a corrupt one being to start it. It is confined to the case where the launcher was missing or truncated, since an intact one never reaches this loop: _launcher_health_error asks the interpreter and reports healthy."""
        ...
        # No copy could be put back and started. A launcher that is simply gone, or one the policy denies, is still not a broken CLI, so ask the interpreter before giving up. Asked only here, after every candidate has been tried, so a launcher that could have been recovered still is.
        return self._recovered_cli_health_error() is None

    def _launcher_runs_error(self) -> Optional[str]:
        """Whether THIS launcher file starts and answers --version. Deliberately about the file, not about the CLI: it is the only check that can catch a stub that is PE-shaped but corrupt, on every machine, and its caller decides what a policy denial means. The stub is probed first, and still is on a locked-down machine, which costs one denied-launch event per update there: a log entry, not a failure. Asking the interpreter instead would be quieter but blind, since it cannot see a corrupt launcher on ANY machine, including the overwhelming majority that have no policy at all, which is the strictly worse trade."""
        assert self.launcher is not None
    ...
    def _launcher_health_error(self) -> Optional[str]:
        """Whether the update left a working CLI, the question that matters. Identical to _launcher_runs_error except when the launcher was denied by an Application Control policy: that denial happens before Python starts, so it says nothing about the update and the package can be perfectly healthy. Ask the signed interpreter beside it instead, or every update on such a machine reports failure and rolls a good install back (issue #8490). A launcher that fails to start for any other reason is the failure it always was."""
    ...
    def _recovered_cli_health_error(self) -> Optional[str]:
        """_launcher_health_error, once recovering the launcher has been ruled out. Absence is the difference: a missing launcher IS worth restoring, so _launcher_health_error keeps reporting it and validate_launcher goes on to put the previous one back. But quarantine deletes the unsigned stub rather than denying it, and no copy survives being restored either, so once every candidate has failed, absence says as little about the update as a policy denial does: ask the interpreter instead of failing a good update and rolling it back (issue #8490)."""
        error = self._launcher_runs_error()
    ...
    def _interpreter_health_error(self, reason: str) -> Optional[str]:
        """Health of the managed CLI when the launcher itself cannot be started. Answers the question validate_launcher actually has, whether the update left a working CLI, on a machine where --version on the launcher can never succeed; falls back to reporting the original block when there is no interpreter to ask. Isolated, alone among this module's managed invocations, because the launch it is predicting is itself isolated: build_update_command in studio/src-tauri/src/update.rs runs the desktop updater under Isolation::Isolated and clears PYTHONHOME/PYTHONPATH, and inheriting them here would let a foreign checkout on PYTHONPATH answer --version for a managed package the update actually broke, so validate_launcher would keep an update that the next desktop launch cannot start. -I implies -E, so it clears the same two variables the Rust side removes by hand."""
        assert self.launcher is not None
        ...
        try:
            result = subprocess.run(
                ...
                check = False,
                capture_output = True,
                # The import probe's ceiling, not the launcher's: --version on the launcher is a process start, while here it is a bare interpreter start plus the whole CLI package import, exactly the work _MANAGED_CLI_IMPORT_PROBE_TIMEOUT is generous for. Under the antivirus scan that produced the quarantine this path exists to survive, the launcher's 10 seconds would call a healthy update broken and roll it back, once per recovery candidate.
                timeout = _MANAGED_CLI_IMPORT_PROBE_TIMEOUT,
                **_windows_hidden_subprocess_kwargs(),
            )
        except subprocess.TimeoutExpired:
    ...
    def _managed_scripts_dir() -> Path:
        """Scripts dir of the venv setup actually updates. setup.ps1 installs into STUDIO_HOME/unsloth_studio, which is not this interpreter when a pip-installed or checkout CLI drives the update. Same distinction _studio_deps._managed_root draws for the damage scan."""
    ...
    def __enter__(self):
        ...
        # Under the Unsloth home, not the venv: setup.ps1 removes the whole $VenvDir to rebuild a stale torch, and an open handle inside it makes Windows refuse the recursive delete. One lock per Unsloth home is the right grain anyway, since that is what names the managed venv.
        self.lock_path = STUDIO_HOME / "unsloth.exe.update-lock"
        # install.ps1 hardlinks this to the launcher, so it survives the old updater's .deleteme unlink and is a valid recovery source.
        ...
        try:
            ...
            if not self._is_valid_pe(self.launcher):
                # Warn, do not exit. The previous updater could leave an install with no launcher and no .deleteme, and refusing here would stop exactly those users from ever updating again. Setup may well write a new launcher; validate_launcher still judges the result.
                typer.echo(
                ...
                # Only write a backup when there is no usable one already. A backup outlives __enter__ only when a previous run died before validating, so it holds the last launcher known to run, while the canonical file has passed nothing but the two-byte header check. Overwriting it here destroyed the only recovery copy.
                try:
                    self._atomic_copy(self.launcher, self.backup)
                except OSError as exc:
                    # A backup is a safety net, not a precondition. Antivirus or a locked-down Scripts dir must not abort the update outright.
    ...
    def validate_launcher(self) -> None:
        ...
        # Only once the launcher is actually back. A quarantined or locked stub can be judged healthy through the interpreter while every attempt to restore a copy failed, and deleting the copies there would throw away the only material a later run could recover from. They are fixed names, so keeping them costs nothing and the next update that does put a launcher back clears them.
        if not self._is_valid_pe(self.launcher):
            return

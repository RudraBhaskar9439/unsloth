...
def _terminal_password_gate(
    *,
    ...
    """Force a terminal password change before the public tunnel goes up. When the tunnel is about to
    publish Unsloth and the seeded admin password was never changed, ask for a new one before any
    public URL exists. The CLI normally does this before re-exec'ing the backend; this is the
    backstop for direct `python run.py` launches and older-CLI installs. Must run BEFORE the uvicorn
    socket binds: on a wildcard bind the served HTML injects the bootstrap credential, so a pre-gate
    listener would hand the default password to anyone reaching the raw port while the operator
    types. Returns (proceed, drop_bootstrap_injection). proceed False aborts the launch (interactive
    refusal, or a headless public launch nothing would protect); drop_bootstrap_injection True means
    the caller must null app.state.bootstrap_password. Without a usable terminal the prompt is
    skipped: proceed if the bootstrap deadline will protect the launch, and refuse if even that is
    disabled (api-only, timeout 0). NOT wrapped in a broad try/except: an auth storage failure must
    abort rather than expose the default."""

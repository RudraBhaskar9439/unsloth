...
# torchao cpp pins ONE torch release AND CUDA major (a CUDA mismatch fails to import): 2.9.x -> 0.14.0, 2.10.x cu<=12 -> 0.16.0, 2.10.x cu>=13 -> 0.17.0, 2.11.x -> 0.17.0 (pytorch/ao#2919).
...
def _select_torchao_spec(torch_version: str | None) -> str:
    ...
    if major != 2:
        return _TORCHAO_DEFAULT_SPEC
    if minor >= 11:
        return _TORCHAO_TORCH_211_PLUS_SPEC
    if minor == 10:
        ...
        try:
            ...
            if not _exact_distribution_spec_is_installed(_spec):
                ...
                if not pip_install_try(
                    ...
                    # Across a CUDA-major move the resident build is not merely slower: a torchao compiled for CUDA 12 cannot load its cpp under cu130, so remove it, as the xFormers arm below does. A release-only move keeps the old warning, since that one really is just the slow path.
...
def install_python_stack() -> int:
    ...
    # macOS arm64: install MLX stack at latest (UV_OVERRIDE relaxes the mlx-vlm / mlx-lm transformers pin, set at module load). Not on a --no-torch install: it declined the training stack, and the runtime's no_torch verdict tells the user an update will not put it back.
    if IS_MAC_ARM and not skip_base and not NO_TORCH:
